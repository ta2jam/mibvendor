...
test("developer mini documentation matches the live public alpha", async () => {
  ...
  assert.match(html, /Read next_cursor from that response and pass it unchanged/);
  assert.match(html, /id="api-live-first"/);
  assert.match(html, /id="api-live-next"[^>]*disabled/);
  assert.match(html, /id="api-live-status"[^>]*role="status"[^>]*aria-live="polite"/);
  assert.match(html, /id="api-live-response"/);
  assert.match(html, /href="\/healthz">Service health/);
  assert.match(html, /href="\/status">Service status/);
  assert.match(html, /data-release-unavailable/);
  assert.match(html, /standard library only/);
  assert.match(app, /navigator\.clipboard\?\.writeText/);
  assert.match(app, /fetch\(`\/v1\/modules\?q=IANA&limit=1&cursor=\$\{cursor\}`/);
  assert.match(app, /body\?\.query !== "IANA"/);
  assert.match(app, /body\.cursor !== cursor/);
  assert.match(app, /apiLiveNextCursor = body\.next_cursor/);
  assert.match(app, /loadLiveModulePage\(apiLiveNextCursor\)/);
  assert.doesNotMatch(app, /fetch\(`\$\{[^}]+\}\/v1\/modules/);
  assert.match(html, /120/);
  assert.doesNotMatch(html, /Synthetic batch resolution example/);
