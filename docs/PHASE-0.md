...
| Parser | One canonical parser selected from reproducible measurements | `experiments/parser-bakeoff/` | Passed: PySMI 2.0.0 is the only candidate meeting the 90% public parse/feature thresholds plus all 9 CC0 expectations and 10/10 fields. Native Linux amd64/arm64 results are deterministic and identical; run `29719084848` and the fail-closed selection record are committed. |
| Prototype | Public task prototype available for sessions | `prototype/` | Passed: [mibvendor.io](https://mibvendor.io) is the production origin; rights-aware module catalog, licensed raw downloads, browser-local walk decoding, dependency states, and the 50,000-line baseline are deployed. The expanded device-identity workbench, RackTables exact-definition layer, and LibreNMS platform-prefix layer are deployed as immutable `v0.4.0-alpha.3` / `device-identity-2026-07-20.3`; tag, commit, image, CI, VPS state, public API/UI checks, and production monitoring are reconciled. |
